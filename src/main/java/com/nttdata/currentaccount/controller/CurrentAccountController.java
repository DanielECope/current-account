...
public class CurrentAccountController {
    ...
    public Flux<CurrentAccount> list(){
        log.info("CurrentAccount: controller list() method ");
        return service.findAll();
    }
    @GetMapping("/list/{id}")
    public Mono<CurrentAccount> findById(@PathVariable String id){
        log.info("CurrentAccount: controller findById() method ");
        return service.findById(id);
    }
    @PostMapping(path = "/create")
    public Mono<CurrentAccount> create(@RequestBody CurrentAccount obj)
    {
        log.info("CurrentAccount: controller create() method : {}",obj.toString());
        return service.save(obj);
    }
    @PutMapping(path = "/update")
    public Mono<CurrentAccount> update(@RequestBody CurrentAccount obj)
    {
        log.info("CurrentAccount: controller update() method : {}",obj.toString());
        return service.update(obj);
    }
    @DeleteMapping(path = "/delete/{id}")
    public Mono<String> delete(@PathVariable String id)
    {
        log.info("CurrentAccount: controller delete() method : {}",id);
